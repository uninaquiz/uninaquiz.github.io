...
import { useHistory, useDeleteHistory } from "@/features/quiz-history/api/use-history";
import { Badge } from "@/shared/ui/atoms";
import type { Difficulty, HistoryItem } from "@/entities/quiz";
...
export const Home: React.FC = () => {
  ...
  const diffConfig: Record<Difficulty, { active: React.CSSProperties }> = {
    easy:   { active: { background: "#003322", borderColor: "#00FF87", color: "#00FF87" } },
    medium: { active: { background: "#332200", borderColor: "#ffb800", color: "#ffb800" } },
    hard:   { active: { background: "#330011", borderColor: "#ff4d6d", color: "#ff4d6d" } },
  };
        ...
        <div className="rounded-2xl p-6" style={{ background: "#121e3d", border: "1px solid #1e3060" }}>
          <div className="flex items-center gap-3 mb-4">
            <span className="text-xs font-bold tracking-widest"
              style={{ color: "#00FF87", fontFamily: "'Orbitron',sans-serif" }}>⏱ CHECKPOINT HISTORY</span>
            <div className="flex-1 h-px" style={{ background: "#1e3060" }} />
          </div>

          {historyLoading ? (
            <p className="text-xs text-center py-4" style={{ color: "#8899bb" }}>Carregando histórico...</p>
          ) : history.length === 0 ? (
